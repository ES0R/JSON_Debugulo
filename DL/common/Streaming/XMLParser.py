...
class XMLParser:
    ...
    def __init__(self):
        ...
        self.decimalExponentDistance = 0 # Decimal exponent for Distance (Visionary T & T Mini) or Z (Visionary S) map
        # "private"
    ...
    def getFrameLengthDepthMap(self):
        ...
        if self._frameLengthDepthMap is not None:
            ...
            logging.error("FrameLength not calculated, since no DepthData is available!")
            return None
    ...
    def getDataFormat(self, xmlNode):
        ...
        knownSizes = { "uint8":1, "uint16":2, "uint32":4, "uint64":8, "float32":4}
    ...
    def calcFrameLengthDepthMap(self):
        ...
        if self.imageWidth is None:
            raise RuntimeError("ERROR: imageWidth is not set. Ensure that the device is streaming depth map data.")
        if self.imageHeight is None:
            raise RuntimeError("ERROR: imageHeight is not set. Ensure that the device is streaming depth map data.")
        ...
        for item in self.dataItems:
            if item["Name"] in knownMapNames:
                self._frameLengthDepthMap += item["Size"] * self.imageWidth * self.imageHeight
            else:
                self._frameLengthDepthMap += item["Size"]

        logging.debug("Calculated framelength (from xml): {} bytes".format(self._frameLengthDepthMap))
            ...
            for formatDescriptionDepthMap in dataSetDepthMap.iter('FormatDescriptionDepthMap'):
                ...
                if self.stereo:
                    for Z in formatDescriptionDepthMap.iter('Z'):
                         ...
                         self.decimalExponentDistance = int(Z.attrib['decimalexponent'])
                    ...
                    for distance in formatDescriptionDepthMap.iter('Distance'):
                        ...
                        self.decimalExponentDistance = int(distance.attrib['decimalexponent'])
